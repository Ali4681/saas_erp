...
const ROLE_CODES = [
  {
    ...
    financialProfile: 'SYSTEM_ADMIN' as const,
  },
  {
    code: 'COMPANY_OWNER',
    name: 'Company Owner',
  ...
  },
  {
    ...
    name: 'Accountant',
    scope: 'TENANT' as const,
    financialProfile: 'ACCOUNTANT' as const,
  },
  {
    code: 'OPERATIONS_MANAGER',
    name: 'Operations Manager',
    scope: 'TENANT' as const,
    financialProfile: 'BRANCH_MANAGER' as const,
  },
  {
  ...
  },
  {
    ...
    scope: 'TENANT' as const,
    financialProfile: 'MANAGER_SUPERVISOR' as const,
  },
  {
    code: 'SALES_DELIVERY_REP',
    name: 'Sales / Delivery Rep',
    scope: 'TENANT' as const,
  ...
  },
  {
    ...
    scope: 'TENANT' as const,
    financialProfile: 'WAREHOUSE_KEEPER' as const,
  },
  {
  ...
  {
    code: 'TREASURY_CUSTODIAN',
    name: 'Treasury Custodian',
    scope: 'TENANT' as const,
    financialProfile: 'TREASURY_CUSTODIAN' as const,
  },
  {
    code: 'BRANCH_MANAGER',
    name: 'Branch Manager',
...
async function main() {
  ...
  for (const role of ROLE_CODES) {
    await prisma.role.upsert({
      ...
      update: {
        ...
        scope: role.scope,
        isSystem: true,
        financialProfile: role.financialProfile,
      },
      create: {
        code: role.code,
        name: role.name,
  ...
  const coaSeed = await seedCompanyChartOfAccounts(prisma, company.id);
  console.log(
    `Chart of accounts: ${coaSeed.accountCount} accounts` +
      (coaSeed.mappingCreated ? ' (mapping created)' : ''),
  );
  ...
  // Always ensure standard operational shifts exist for demo-co
  const { seedStandardWorkShifts } = await import('./seed-standard-shifts');
  const standardShifts = await seedStandardWorkShifts(prisma, company.id);
  console.log(
    `Standard work shifts: ${standardShifts.shiftCount} shifts, ${standardShifts.windowCount} dynamic windows (profile ${standardShifts.profileId})`,
  );
